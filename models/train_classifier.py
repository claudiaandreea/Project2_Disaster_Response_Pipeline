...
def load_data(database_filepath):
    ...
    '''
    engine = create_engine('sqlite:///{}'.format(database_filepath)) #database_filepath
    df = pd.read_sql_table('disaster', con=engine) #database_filepath
    X = df['message']
    Y = df.iloc[:,4:]
...
def evaluate_model(model, X_test, Y_test, category_names): #modification Y_test to y_test
    '''
    Args: 
    model - the build model
    X_test - the independent variables (for the test dataset)
    Y_test - the dependent variables (for the test dataset)
    category_names -  the names categories
...
def save_model(model, model_filepath):
    ...
    Args: 
    model - the build model
    model_filepath - the model location  
    '''
